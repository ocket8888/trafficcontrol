...
\maketitle{}
\newpage{}
\abstract{This document is intended to serve as a blueprint to be followed when implementing the 3.0 version of the Traffic Ops API, as
well as a set of guidelines to be followed when implementing the API routes herein defined and any subsequently added API endpoints. Plugin
developers may also find the guidelines useful, though obviously any code not directly contributed to the project is outside of this document's
purview.\\
For Traffic Ops API version 3.0, several changes to the API model were discussed including, but not limited to, switching from a REST structure
to GraphQL, breaking the API apart into multiple separate microservices or even separate APIs, and doing away with cookie-based authentication.
These ideas have not been adopted and the discussions regarding them tabled for a later date in favor of making the already sweeping changes
herein outlined no more disruptive than they absolutely need to be and easing the transition of clients.}
\newpage{}
\tableofcontents{}
\newpage{}
\section{Guidelines}
...
mean it will be included in 3.0, but the standards defined here should be enough for clients to start building out support for it.} as
defined by \href{https://tools.ietf.org/html/rfc7519}{RFC7519}, signed with JSON Web-Signatures (JWS). If a client requests a resource and
is denied due to failed authentication, the endpoint MUST return a \code{401 Unauthorized} response code that contains an appropriate Alert
as described in Section \ref{sec:401}, and also MUST set the "WWW-Authenticate" header to \emph{exactly}
\code{Bearer realm="Apache Traffic Control"} by default. The realm may be configurable.\\
...
\begin{itemize}
	...
	\item \code{exp} The timestamp at which this JWT will expire. Unlike other dates and times dealt with through the Traffic Ops API, this
...
\subsection{\code{/cache\_servers/\{\{Cache Server ID\}\}}}

\subsection{\code{/delivery\_services}}

\subsection{\code{/delivery\_services/\{\{Delivery Service Name\}\}}}

\subsection{\code{/servers}}

\subsection{\code{/servers/\{\{Server ID\}\}}}

\subsection{\code{/login}\label{sec:endpoint:login}}

\subsection{\code{/origins}}

\subsection{\code{/origins/\{\{Origin ID\}\}}}

\subsection{\code{/physical\_locations}}

\subsection{\code{/physical\_locations/\{\{Physical Location Name\}\}}}
...
\subsection{\code{/roles/\{\{Role Name\}\}}}

\subsection{\code{/tags}}

\subsection{\code{/tags/\{\{Tag\}\}}}

\subsection{\code{/tenants}}

\subsection{\code{/tenants/\{\{Tenant Name\}\}}}
...
\subsection{\code{/users/\{\{Username\}\}}}
