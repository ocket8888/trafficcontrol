...
(FQDN) as a string. For example, a Cache Server with an FQDN of \code{github.com}
has a Domain of \code{com} and a Cache Server with an FQDN of
...
\begin{itemize}
	...
	\item \code{REPORTED} - The Cache Server's health is presented to the Traffic
	Router(s) as it is reported by its various thresholds, as determined by the
	Traffic Monitor(s).
	\item \code{OFFLINE} - The Cache Server is considered unhealthy regardless of
...
\begin{itemize}
	\item \code{EDGE} - This is an Edge-tier Cache Server which acts as a reverse
	proxy.
...
\begin{itemize}
	...
	\item \code{INACTIVE} A Delivery Service that is "inactive" is not available for
