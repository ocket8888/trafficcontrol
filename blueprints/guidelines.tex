...
\begin{itemize}
	\item \code{count} Count contains an unsigned integer that defines the total
		...
		\ref{sec:pagination}).
	\item \code{max} For collections of numeric data points (possibly with other
		associated data e.g. timestamps), this can be used to specify the
		maximum value within the collection specified by the non-pagination
		query parameters supplied by the client. Its type MUST be numeric and
		MUST match the type of the data points it describes.
	\item \code{mean} For collections of numeric data points (possibly with
		other associated data e.g. timestamps), this can be used to specify the
		arithmetic mean of the values within the collection specified by the
		non-pagination query parameters supplied by the client. Its type MUST be
		numeric and MUST match the type of the data points it describes.
	\item \code{min} For collections of numeric data points (possibly with other
		associated data e.g. timestamps), this can be used to specify the
		minimum value within the collection specified by the non-pagination
		query parameters supplied by the client. Its type MUST be numeric and
		MUST match the type of the data points it describes.
\end{itemize}
