...
\begin{codelisting}
\captionof{listing}{Profile Object as a TypeScript Interface}
\begin{minted}[tabsize=2]{typescript}
interface Parameter {
	configurationFile: Exclude<string, "">;
	name: Exclude<string, "">;
	secure: boolean;
...
interface Profile {
	bandwidthThreshold: bigint | null;
	cdn: string;
	description: string;
	hardDisks: Set<string>;
	historyCount: bigint | null;
	healthConnectionTimeout: bigint | null;
	healthPollingPath: Exclude<string, "">;
	healthPollingFormat: string | null;
	loadavgThreshold: number | null;
	name: Exclude<string, "">;
	parameters: Set<Parameter>;
	queryTimeThreshold: bigint | null;
	ramDisks: Set<string>;
	tags: Set<string>;
	type: "EDGE" | "MID";
}
...
\subsubsection{Health Connection Timeout}
This is an unsigned integer number of milliseconds Traffic Monitor will wait
before giving up on a request for health polling information for Cache Servers
using this Profile. If it is "null"-typed, Traffic Monitor is instructed to use
its pre-configured default timeout, which is not defined by the Traffic Ops API.
