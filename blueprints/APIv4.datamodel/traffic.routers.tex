...
\begin{codelisting}
\captionof{listing}{Traffic Router Object as a TypeScript Interface}
\begin{minted}[tabsize=2]{typescript}
interface TrafficRouter {
	apiPort: bigint;
	cacheControlMaxAge: bigint;
	cdn: string;
	coverageZonePollingInterval: bigint;
	deepCoverageZonePollingInterval: bigint;
	dnssec: {
		allowExpiredKeys: boolean;
		effectiveMultiplier: bigint | null;
		fetchInterval: bigint | null;
		fetchRetries: bigint | null;
		fetchTimeout: bigint | null;
		fetchWait: bigint | null;
		generationMultiplier: bigint | null;
		zoneComparisons: boolean;
	};
	domain: string;
	dynamicZoneCachePriming: {
		prime: boolean;
		primingLimit: bigint | null;
	}
	edns0ClientSubnetEnabled: boolean;
	forcedDiversity: boolean;
	geolocationPollingInterval: bigint | null;
	hostName: string;
	id: bigint;
	notes: string;
	online: boolean;
	...
	secureAPIPort: bigint;
	steeringPollingInterval: bigint | null;
	ttls: Map<string, bigint>;
	zones: {
		cacheMaintenanceInterval: bigint | null;
		dynamicInitialCapacity: bigint | null;
		dynamicResponseExpiration: bigint | null;
		initializationTimeout: bigint | null;
		threadCount: bigint | null;
		threadMultiplier: number| null ;
	}
}
\end{minted}
\end{codelisting}
...
are the Time-to-Live values - in seconds - that Traffic Router will use for the
named DNS record type.\\
